...
type Actions = {
  ...
  setToggle: (
    program: Program<Anderdzi>,
    owner: PublicKey,
    key: "watcherEnabled" | "stakingEnabled",
    val: boolean,
    connection: Connection,
  ) => Promise<void>;
  connectTelegram: (
...
export const useVaultStore = create<State & Actions>((set, get) => ({
  ...
  connect: async (pubkey, connection) => {
    set({ connected: true, walletAddress: pubkey.toBase58(), vaultLoading: true });
    const vault = await fetchVault(connection, pubkey);
    set({ vault, vaultLoading: false });
  },

  disconnect: () =>
    set({ connected: false, walletAddress: null, vault: null, vaultLoading: false }),
  ...
  deposit: async (program, owner, amount, connection) => {
    await runTx(`Deposited ${amount} SOL`, "deposit", set, () =>
      ix.deposit(program, owner, amount),
    );
    await refreshVault(connection, owner, set);
  },

  withdraw: async (program, owner, amount, connection) => {
    await runTx(`Withdrew ${amount} SOL`, "withdraw", set, () =>
      ix.withdraw(program, owner, amount),
    );
    await refreshVault(connection, owner, set);
  },
