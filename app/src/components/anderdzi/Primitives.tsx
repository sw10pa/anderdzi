...
export function PillButton({ variant = "primary", loading, icon, children, className, disabled, fullWidth, ...rest }: BtnProps) {
  const base = "inline-flex items-center justify-center gap-2 rounded-[var(--r)] px-4 py-2 text-sm font-semibold transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed";
  const variants = {
    primary: "bg-[var(--accent)] text-[#0F3932] hover:brightness-110 hover:shadow-[0_0_24px_rgba(74,255,145,0.4)]",
    secondary: "border border-[var(--border)] bg-transparent text-[var(--text)] hover:bg-[var(--surface-2)]",
    danger: "border border-[var(--danger)] text-[var(--danger)] bg-transparent hover:bg-[rgba(255,107,107,0.15)]",
  };
  return (
    <button {...rest} disabled={disabled || loading} className={cn(base, variants[variant], fullWidth && "w-full", className)}>
...
export function IconButton({ children, label, active, danger, ...rest }: ButtonHTMLAttributes<HTMLButtonElement> & { label: string; active?: boolean; danger?: boolean }) {
  return (
  ...
  return (
    <button
      ...
      className={cn(
        ...
        checked ? "bg-[#1F8A4C]" : "bg-[var(--surface-2)]",
      )}
    >
      <span
        className={cn(
...
export function Badge({ kind, children }: { kind: "ACTIVE" | "TRIGGERED" | "DISTRIBUTED"; children?: ReactNode }) {
  const map = {
  ...
  const s = map[kind];
  return (
    <span className="inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-widest" style={{ background: s.bg, color: s.color, boxShadow: kind === "ACTIVE" ? "0 0 16px rgba(74,255,145,0.35)" : undefined }}>
      <span className="anim-pulse-dot inline-block h-1.5 w-1.5 rounded-full" style={{ background: s.color }} />
      {children ?? kind}
    </span>
  );
}
...
export function Slider({ value, onChange, min, max, step = 1 }: { value: number; onChange: (v: number) => void; min: number; max: number; step?: number }) {
  ...
  return (
    <div className="relative">
      <div className="absolute top-1/2 left-0 h-1.5 -translate-y-1/2 rounded-full bg-[var(--accent)]" style={{ width: `${pct}%` }} />
      <input
        type="range"
